...
void analVars(NodeFunc fn){
	...
	static immutable string[knownFieldsCT.length] knownFields = knownFieldsCT;
			...
			selectNode!(void)(nn,
				(NodeVarDecl n){
					foreach(immutable idx, string name; n.names){
						if((name in locals) || (name in globals)) message(fn, n.locs[idx], ": duplicate variable definition: '", name, "'");
						if(n.asGlobal){
							globals[name] = VarInfo(VarInfo.Type.Global, name);
						}else{
				...
				(NodeId n){
					if(!isCompound){
						if(n.name !in locals && n.name !in globals && n.name !in fields){
							...
							}else message(fn, n.loc, ": undefined variable access: '", n.name, "'");
